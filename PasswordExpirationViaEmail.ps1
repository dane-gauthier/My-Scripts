...
###################################################################
foreach ($user in (Get-ADGroupMember -Identity "OU OF USERS TO APPLY TO.  CAN CHANGE TO APPLY TO ENTIRE DOMAIN")) {
...
If you have any issues, please feel free to contact Secant Support and we can assist with the process. 
...
"@
    if ($EmailBody -ne $null) {
        Send-MailMessage -from 'no-reply@contoso.com' -To $UserEmailAddress -Subject 'Password Expiration Notification' -Body $EmailBody -SmtpServer $PSEmailServer
        }
    }
